...
import { CheckCircle2, XCircle, Info, Volume2, VolumeX } from "lucide-react"
...
export function DailyChallenge({
  ...
  const handleTryAnother = async () => {
    ...
    try {
      // Fetch a new question without reloading the page
      // Use mode=random to get a different question
      const response = await fetch("/api/daily-challenge?mode=random", {
        cache: "no-store",
      })
      ...
      // Update the component with the new question
      setQuestion(newChallenge.question)
      setOptions(newChallenge.options)
      setCorrectAnswer(newChallenge.correct_answer)
      setExplanation(newChallenge.explanation)
      setTopic(newChallenge.topic)
    ...
    <div className="bg-white/10 backdrop-blur-sm rounded-lg p-6 mb-6">
      ...
      {!isSubmitted ? (
        ...
        <div className="mt-6">
          ...
          >
            {isLoadingNew ? "Loading..." : "Try Another Question"}
          </Button>
        </div>
      )}
    </div>
